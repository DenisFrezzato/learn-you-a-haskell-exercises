...
-- create a list of length n of the Fibonaci sequence in reverse order
-- examples: fib 0 = [0]
-- 	     fib 1 = [1, 0]
...
stepReverseSign n step = sign * (abs n + step)
    where
...
piCalc' :: (Ord a, Fractional a, Integral b) => a -> a -> a -> b -> (a, b)
piCalc' denominator prevPi tolerance executionCount = if abs (newPi - prevPi) < tolerance
    then (newPi, executionCount)
    else piCalc' (stepReverseSign denominator 2) newPi tolerance (executionCount + 1)
    where
        newPi = 4 / denominator + prevPi 
